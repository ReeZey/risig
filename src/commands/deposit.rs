...
pub(crate) async fn run(user: User, mut user_data: Document, options: &Vec<CommandDataOption>) -> CommandResponse {
    ...
    if amount > money {
        return CommandResponse::new(format!("you dont have enough money [{} < {}]", money, amount), true);
    }
    ...
    return CommandResponse::new(format!("you deposited `{} ris`, now you have `{} ris` in the bank", amount, bank_money + amount), false);
}