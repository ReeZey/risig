...
pub(crate) async fn run(command: &mut ApplicationCommandInteraction, ctx: &Context, user: User, user_data: Document) {
    ...
    if pretty_json_data.len() > 2000 {
        pretty_json_data = format!("{}{}", &pretty_json_data[..1997], "...");
    }
    
    //send_command_response(command, &ctx, &format!("```json\n{}```", pretty_json_data), MessageFlags::EPHEMERAL).await;
    send_file_command_response(command, &ctx, &pretty_json_data, (pretty_json_data.as_bytes().to_vec(), &format!("{}.json", user.name)), MessageFlags::EPHEMERAL).await;
