...
pub(crate) async fn run(command: &mut ApplicationCommandInteraction, ctx: &Context, user: User, user_data: Document) {
    ...
    send_file_command_response(command, &ctx, &format!("```json\n{}```", &pretty_json_data), (pretty_json_data.as_bytes().to_vec(), &format!("{}.json", user.name)), MessageFlags::EPHEMERAL).await;
